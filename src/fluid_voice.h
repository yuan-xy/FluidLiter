...
struct _fluid_voice_t {
    ...
    int end;
    int loopstart;
    int loopend; /* Note: first point following the loop (superimposed on
                    loopstart) */
    ...
    fluid_real_t modlfo_incr;  /* the lfo frequency is converted to a per-buffer
                                  increment */
    ...
    fluid_real_t viblfo_incr;  /* the lfo frequency is converted to a per-buffer
                                  increment */
...
int fluid_voice_write(fluid_voice_t *voice, fluid_real_t *left, fluid_real_t *right,
                      fluid_real_t* reverb_buf, fluid_real_t* chorus_buf);

int fluid_voice_init(fluid_voice_t *voice, fluid_sample_t *sample, fluid_channel_t *channel,
                     int key, int vel, unsigned int id, unsigned int time, fluid_real_t gain);
...
int calculate_hold_decay_buffers(fluid_voice_t *voice, int gen_base, int gen_key2base,
                                 int is_decay);
...
#define fluid_voice_set_id(_voice, _id)                                                            \
    {                                                                                              \
        (_voice)->id = (_id);                                                                      \
    }
...
#define _PLAYING(voice)                                                                            \
    (((voice)->status == FLUID_VOICE_ON) || ((voice)->status == FLUID_VOICE_SUSTAINED))

/* A voice is 'ON', if it has not yet received a noteoff
 * event. Sending a noteoff event will advance the envelopes to
 * section 5 (release). */
#define _ON(voice)                                                                                 \
    ((voice)->status == FLUID_VOICE_ON && (voice)->volenv_section < FLUID_VOICE_ENVRELEASE)
...
#define _GEN(_voice, _n)                                                                           \
    ((fluid_real_t)(_voice)->gen[_n].val + (fluid_real_t)(_voice)->gen[_n].mod +                   \
