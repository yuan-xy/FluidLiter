...
struct _fluid_voice_t {
    ...
    unsigned char
        chan; /* the channel number, quick access for channel messages */
    ...
    int check_sample_sanity_flag; /* Flag that initiates, that sample-related
    ...
    fluid_real_t
        min_attenuation_cB; /* Estimate on the smallest possible attenuation
    ...
    /* resonant filter */
    fluid_real_t
        fres; /* the resonance frequency, in cents (not absolute cents) */
...
int fluid_voice_write(fluid_voice_t *voice, fluid_real_t *left,
                      fluid_real_t *right, fluid_real_t *reverb_buf);
