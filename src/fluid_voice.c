...
int fluid_voice_init(fluid_voice_t *voice, fluid_sample_t *sample,
    ...
    voice->has_looped = 0; /* Will be set during voice_write when the 2nd loop
                              point is reached */
    voice->last_fres = -1; /* The filter coefficients have to be calculated
                              later in the DSP loop. */
    voice->filter_startup = 1; /* Set the filter immediately, don't fade between
...
int fluid_voice_write(fluid_voice_t *voice, fluid_real_t *dsp_left_buf,
    ...
    if (voice->volenv_section == FLUID_VOICE_ENVATTACK) {
         ...
         * A positive modlfo_to_vol should increase volume (negative
         * attenuation).
         */
        ...
        /* If the loop amplitude is known, we can use it if the voice loop is
    ...
    /* Volume increment to go from voice->amp to target_amp in FLUID_BUFSIZE
     * steps */
    voice->amp_incr = (target_amp - voice->amp) / FLUID_BUFSIZE;

    /* no volume and not changing? - No need to process */
    if ((voice->amp == 0.0f) && (voice->amp_incr == 0.0f)) goto post_process;
    ...
    if ((fabs(fres - voice->last_fres) > 0.01)) {
         ...
         *
         * Here a couple of multiplications are saved by reusing common
         * expressions. The original equations should be:
...
#define FILTER_TRANSITION_SAMPLES (FLUID_BUFSIZE)
            ...
            voice->b02_incr =
                (b02_temp - voice->b02) / FILTER_TRANSITION_SAMPLES;
...
/* Purpose:
 *
 * - filters (applies a lowpass filter with variable cutoff frequency and
 * quality factor)
...
static void fluid_voice_effects(fluid_voice_t *voice, int count,
    ...
    if (dsp_filter_coeff_incr_count > 0) {
        ...
        for (dsp_i = 0; dsp_i < count; dsp_i++) {
            ...
            dsp_centernode =
                dsp_buf[dsp_i] - dsp_a1 * dsp_hist1 - dsp_a2 * dsp_hist2;
            dsp_buf[dsp_i] =
...
void fluid_voice_start(fluid_voice_t *voice) {
     ...
     * sample with its nominal loop settings. This happens, when the sample is
...
#define __pitch(_k) fluid_tuning_get_pitch(tuning, _k)
        ...
        voice->gen[GEN_PITCH].val =
            (__pitch(60) + (voice->gen[GEN_SCALETUNE].val / 100.0f *
    ...
    /* Make an estimate on how loud this voice can get at any time
     * (attenuation). */
    voice->min_attenuation_cB =
...
int calculate_hold_decay_buffers(fluid_voice_t *voice, int gen_base,
     ...
     */
    timecents = (_GEN(voice, gen_base) +
                 _GEN(voice, gen_key2base) * (60.0 - voice->key));
...
void fluid_voice_update_param(fluid_voice_t *voice, int gen) {
    ...
    case GEN_PAN:
        ...
        voice->amp_left =
            fluid_pan(voice->pan, 1) * voice->synth_gain / 32768.0f;
        voice->amp_right =
            fluid_pan(voice->pan, 0) * voice->synth_gain / 32768.0f;
        break;
    ...
    case GEN_FINETUNE:
        ...
        voice->pitch =
            (_GEN(voice, GEN_PITCH) + 100.0f * _GEN(voice, GEN_COARSETUNE) +
    ...
    case GEN_OVERRIDEROOTKEY:
        /* This is a non-realtime parameter. Therefore the .mod part of the
         * generator can be neglected. NOTE: origpitch sets MIDI root note while
         * pitchadj is a fine tuning amount which offsets the original rate.
         * This means that the fine tuning is inverted with respect to the root
         * note (so subtract it, not add).
         */
        if (voice->gen[GEN_OVERRIDEROOTKEY].val >
            -1) { // FIXME: use flag instead of -1
            voice->root_pitch = voice->gen[GEN_OVERRIDEROOTKEY].val * 100.0f -
                                voice->sample->pitchadj;
        } else {
            voice->root_pitch =
                voice->sample->origpitch * 100.0f - voice->sample->pitchadj;
        }
    ...
    case GEN_MODLFOFREQ:
        /* - the frequency is converted into a delta value, per buffer of
        ...
        voice->modlfo_incr =
            (4.0f * FLUID_BUFSIZE * fluid_act2hz(x) / voice->output_rate);
        break;
    ...
    case GEN_STARTADDRCOARSEOFS: /* SF2.01 section 8.1.3 # 4 */
        if (voice->sample != NULL) {
            voice->start =
                (voice->sample->start + (int)_GEN(voice, GEN_STARTADDROFS) +
    ...
    case GEN_ENDADDRCOARSEOFS: /* SF2.01 section 8.1.3 # 12 */
        if (voice->sample != NULL) {
            voice->end =
                (voice->sample->end + (int)_GEN(voice, GEN_ENDADDROFS) +
    ...
    case GEN_ENDLOOPADDRCOARSEOFS: /* SF2.01 section 8.1.3 # 50 */
        if (voice->sample != NULL) {
            voice->loopend =
                (voice->sample->loopend + (int)_GEN(voice, GEN_ENDLOOPADDROFS) +
...
#define NUM_BUFFERS_RELEASE(_v)                                                \
    (unsigned int)(voice->output_rate * fluid_tc2sec_release(_v) /             \
                   FLUID_BUFSIZE)
         ...
         * - attack, decay and release are converted to their increment per
         * sample
         */
    ...
    case GEN_VOLENVATTACK: /* SF2.01 section 8.1.3 # 34 */
        ...
        voice->volenv_data[FLUID_VOICE_ENVATTACK].incr =
    ...
    case GEN_KEYTOVOLENVHOLD: /* SF2.01 section 8.1.3 # 39 */
        count = calculate_hold_decay_buffers(
            voice, GEN_VOLENVHOLD, GEN_KEYTOVOLENVHOLD, 0); /* 0 means: hold */
    ...
    case GEN_KEYTOVOLENVDECAY: /* SF2.01 section 8.1.3 # 40 */
        ...
        count = calculate_hold_decay_buffers(
            voice, GEN_VOLENVDECAY, GEN_KEYTOVOLENVDECAY, 1); /* 1 for decay */
        voice->volenv_data[FLUID_VOICE_ENVDECAY].count = count;
        voice->volenv_data[FLUID_VOICE_ENVDECAY].coeff = 1.0f;
        voice->volenv_data[FLUID_VOICE_ENVDECAY].incr =
            count ? -1.0f / count : 0.0f;
    ...
    case GEN_VOLENVRELEASE: /* SF2.01 section 8.1.3 # 38 */
        ...
        voice->volenv_data[FLUID_VOICE_ENVRELEASE].incr =
            count ? -1.0f / count : 0.0f;
    ...
    case GEN_MODENVATTACK: /* SF2.01 section 8.1.3 # 26 */
        ...
        voice->modenv_data[FLUID_VOICE_ENVATTACK].incr =
            count ? 1.0f / count : 0.0f;
    ...
    case GEN_KEYTOMODENVHOLD: /* SF2.01 section 8.1.3 # 31 */
        count = calculate_hold_decay_buffers(
            voice, GEN_MODENVHOLD, GEN_KEYTOMODENVHOLD, 0); /* 1 means: hold */
    ...
    case GEN_KEYTOMODENVDECAY: /* SF 2.01 section 8.1.3 # 32 */
        count = calculate_hold_decay_buffers(
            voice, GEN_MODENVDECAY, GEN_KEYTOMODENVDECAY, 1); /* 1 for decay */
        ...
        voice->modenv_data[FLUID_VOICE_ENVDECAY].incr =
            count ? -1.0f / count : 0.0f;
    ...
    case GEN_MODENVRELEASE: /* SF 2.01 section 8.1.3 # 30 */
        ...
        voice->modenv_data[FLUID_VOICE_ENVRELEASE].incr =
 ...
 * @param cc flag to distinguish between a continous control and a channel
 * control (pitch bend, ...)
        ...
        if (fluid_mod_has_source(mod, cc, ctrl)) {
            ...
            for (k = 0; k < voice->mod_count; k++) {
                if (fluid_mod_has_dest(&voice->mod[k], gen)) {
                    modval += fluid_mod_get_value(&voice->mod[k],
                                                  voice->channel, voice);
                }
            }
...
int fluid_voice_noteoff(fluid_voice_t *voice) {
    ...
    if (voice->channel && fluid_channel_sustained(voice->channel)) {
        ...
        if (voice->volenv_section == FLUID_VOICE_ENVATTACK) {
             ...
             * amplitude. The other sections use logarithmic scaling. Calculate
             * new volenv_val to achieve equievalent amplitude during the
             * release phase for seamless volume transition.
             */
 ...
 * mode == FLUID_VOICE_OVERWRITE: Identical modulators on instrument level are
 * overwritten mode == FLUID_VOICE_DEFAULT: This is a default modulator, there
 * can be no identical modulator. Don't check.
 */
void fluid_voice_add_mod(fluid_voice_t *voice, fluid_mod_t *mod, int mode) {
    ...
    if (((mod->flags1 & FLUID_MOD_CC) == 0) &&
        ...
        FLUID_LOG(FLUID_WARN,
                  "Ignoring invalid controller, using non-CC source %i.",
                  mod->src1);
        return;
    }
    ...
    if (mode == FLUID_VOICE_ADD) {
        ...
        /* if identical modulator exists, replace it (only the amount has to be
         * changed) */
        for (i = 0; i < voice->mod_count; i++) {
            if (fluid_mod_test_identity(&voice->mod[i], mod)) {
    ...
    for (i = 0; i < voice->mod_count; i++) {
        ...
        if ((mod->dest == GEN_ATTENUATION) &&
            ...
            if ((mod->src1 == FLUID_MOD_PITCHWHEEL) ||
                ...
                (mod->flags2 & FLUID_MOD_BIPOLAR) || (mod->amount < 0)) {
                /* Can this modulator produce a negative contribution? */
                v *= -1.0;
            } else {
                /* No negative value possible. But still, the minimum
                 * contribution is 0. */
                v = 0;
            }
...
void fluid_voice_check_sample_sanity(fluid_voice_t *voice) {
    ...
    if ((_SAMPLEMODE(voice) == FLUID_LOOP_UNTIL_RELEASE) ||
        ...
        if (voice->loopstart > voice->loopend) {
            ...
            voice->loopstart = voice->loopend;
            voice->loopend = temp;
            /*FLUID_LOG(FLUID_DBG, "Loop / sample sanity check: Changing order
             * of loop points!"); */
        }
        ...
        /* The loop points may have changed. Obtain a new estimate for the loop
         * volume. */
    ...
    if (((_SAMPLEMODE(voice) == FLUID_LOOP_UNTIL_RELEASE) &&
        ...
        if (index_in_sample >= voice->loopend) {
            /* FLUID_LOG(FLUID_DBG, "Loop / sample sanity check: Phase after 2nd
             * loop point!"); */
            fluid_phase_set_int(voice->phase, voice->loopstart);
        }
    }
    /*    FLUID_LOG(FLUID_DBG, "Loop / sample sanity check: Sample from %i to
     * %i, loop from %i to %i", voice->start, voice->end, voice->loopstart,
     * voice->loopend); */
...
int fluid_voice_set_param(fluid_voice_t *voice, int gen,
                          fluid_real_t nrpn_value, int abs) {
