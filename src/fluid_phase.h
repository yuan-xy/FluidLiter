 ...
 * The fractional position of the playing pointer is calculated with a quite
 * high resolution (32 bits). It would be unpractical to keep a set of
 * interpolation coefficients for each possible fractional part...
 */
#define fluid_phase_fract_to_tablerow(_x)                                      \
    ((unsigned int)(fluid_phase_fract(_x) & FLUID_INTERP_BITS_MASK) >>         \
     FLUID_INTERP_BITS_SHIFT)

#define fluid_phase_double(_x)                                                 \
    ((double)(fluid_phase_index(_x)) +                                         \
     ((double)fluid_phase_fract(_x) / FLUID_FRACT_MAX))
