...
int fluid_dsp_float_interpolate_linear(fluid_voice_t *voice) {
    ...
    if (looping)
        ...
        point = dsp_data[voice->end]; /* duplicate end for samples no longer
                                         looping */
    ...
    while (1) {
        ...
        for (; dsp_i < FLUID_BUFSIZE && dsp_phase_index <= end_index; dsp_i++) {
            ...
            dsp_buf[dsp_i] =
                dsp_amp * (coeffs[0] * dsp_data[dsp_phase_index] +
...
int fluid_dsp_float_interpolate_4th_order(fluid_voice_t *voice) {
            ...
            ->has_looped) /* set start_index and start point if looped or not */
    {
        ...
        start_point =
            dsp_data[voice->loopend - 1]; /* last point in loop (wrap around) */
    } else {
    ...
    while (1) {
        ...
        for (; dsp_i < FLUID_BUFSIZE && dsp_phase_index <= end_index; dsp_i++) {
            coeffs = interp_coeff[fluid_phase_fract_to_tablerow(dsp_phase)];
            dsp_buf[dsp_i] =
                dsp_amp * (coeffs[0] * dsp_data[dsp_phase_index - 1] +
                           coeffs[1] * dsp_data[dsp_phase_index] +
                           coeffs[2] * dsp_data[dsp_phase_index + 1] +
                           coeffs[3] * dsp_data[dsp_phase_index + 2]);
