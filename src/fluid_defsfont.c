...
int fluid_defsfont_add_preset(fluid_defsfont_t *sfont, fluid_defpreset_t *preset)
{
  ...
  else
  {
    ...
    while (cur != NULL)
    {
      if ((preset->bank < cur->bank) || ((preset->bank == cur->bank) && (preset->num < cur->num)))
      {
        if (prev == NULL)
        {
          preset->next = cur;
 ...
 */
int fluid_defsfont_load_sampledata(fluid_defsfont_t *sfont, fluid_fileapi_t *fapi)
{
  fluid_file fd;
      ...
      while (inst_zone != NULL){
        ...
        if (fluid_inst_zone_inside_range(inst_zone, key, vel) && (sample != NULL)){
          ...
          for (i = 0; i < GEN_LAST; i++){
            ...
            if ((i != GEN_STARTADDROFS) && (i != GEN_ENDADDROFS) && (i != GEN_STARTLOOPADDROFS) && (i != GEN_ENDLOOPADDROFS) && (i != GEN_STARTADDRCOARSEOFS) && (i != GEN_ENDADDRCOARSEOFS) && (i != GEN_STARTLOOPADDRCOARSEOFS) && (i != GEN_KEYNUM) && (i != GEN_VELOCITY) && (i != GEN_ENDLOOPADDRCOARSEOFS) && (i != GEN_SAMPLEMODE) && (i != GEN_EXCLUSIVECLASS) && (i != GEN_OVERRIDEROOTKEY))
            {
